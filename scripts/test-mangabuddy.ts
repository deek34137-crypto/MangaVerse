#!/usr/bin/env tsx
/**
 * Dedicated MangaBuddy Provider Test Script
 *
 * Usage:
 *   npx tsx scripts/test-mangabuddy.ts
 */

import "@/services/providers"; // Register all providers
import { providerRegistry } from "@/services/providers/registry";
import { MangaBuddyProvider } from "@/services/providers/mangabuddy/provider";

async function runMangaBuddyTest() {
  console.log("\n🧪  Running Dedicated MangaBuddy Provider Test...\n");

  const provider = providerRegistry.get("mangabuddy") as MangaBuddyProvider;
  if (!provider) {
    console.error("❌ MangaBuddy provider not registered in providerRegistry.");
    process.exit(1);
  }
  ...
  // Test 3: Manga Detail
  const mangaId = searchResults[0]?.id || "prince-of-ovale.ZDI8-Q";
  ...
  const chapterId = chapters[0]?.id ? `${mangaId}/${chapters[0].id}` : `${mangaId}/chapter-1`;
  ...
  console.log("\n✅ MangaBuddy Test Execution Completed.\n");
}

runMangaBuddyTest().catch((err) => {
