...
interface RecentlyViewedItem extends Manga {
  progress?: number;
  chapterId?: string;
  readAt?: string;
  currentChapter?: {
    ...
    return (
      <section className="space-y-4" aria-labelledby="recently-viewed-heading">
        ...
        <div className="bg-ink-900/40 border border-ink-800 rounded-2xl p-8 text-center max-w-lg mx-auto">
          ...
          <h3 className="text-sm font-bold text-foreground mb-1">No Recent History</h3>
          <p className="text-xs text-ink-400 mb-5 leading-normal max-w-sm mx-auto">
            Start reading a manga and it will appear here so you can easily return to it.
          </p>
          <Link href="/search">
                ...
                >
                  ...
                  <div className="mt-2 px-0.5 space-y-1 text-left flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between text-[9px] text-ink-500 mt-1.5 pt-1 border-t border-ink-800/40">
                      <span>{item.progress !== undefined ? `${Math.round(item.progress)}% done` : "Recent"}</span>
                      <span className="flex items-center gap-1">
                        <Clock className="h-2.5 w-2.5 text-ink-500" />
                        {item.readAt ? formatRelativeTime(item.readAt) : "Recently"}
                      </span>
                    </div>
                  </div>
                </Link>
