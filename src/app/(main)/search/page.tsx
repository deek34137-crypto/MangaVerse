...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; sort?: string }> }) {
      ...
      <div className="min-h-screen flex flex-col bg-background text-foreground">
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <div className="max-w-md text-center bg-card border border-border p-8 rounded-2xl shadow-xl">
            ...
            <p className="text-muted-foreground text-sm mb-6">{viewModel.errorMessage}</p>
            <div className="flex gap-3 justify-center">
              <Link href="/" prefetch={true} className="px-5 py-2.5 bg-card border border-border text-foreground text-xs font-semibold rounded-xl hover:bg-accent transition">
                Back to Home
              </Link>
              <Link href="/search" prefetch={true} className="px-5 py-2.5 bg-primary text-primary-foreground text-xs font-semibold rounded-xl hover:bg-primary/90 transition">
                Retry Search
              </Link>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <main className="flex-1 container-padded pt-28 pb-16">
        {/* Back to Home Navigation Button */}
        <div className="mb-6">
          <Link
            href="/"
            prefetch={true}
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-card/80 border border-border/80 hover:border-primary/50 text-xs font-medium text-muted-foreground hover:text-foreground transition-all shadow-sm group"
          >
        ...
        <div className="mb-8 max-w-2xl">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-primary">Catalog Directory</span>
          </div>
          ...
          <form action="/search" method="GET" className="flex gap-3">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl bg-card border border-border text-foreground text-sm focus:outline-none focus:border-primary transition shadow-sm"
            />
            <button
              type="submit"
              className="px-6 py-3 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-bold text-sm transition-all cursor-pointer shadow-md"
            >
              Search
            </button>
          </form>
        ...
        {viewModel.showZeroResults && (
          <div className="p-8 rounded-2xl bg-card border border-border text-center max-w-lg mx-auto my-12 shadow-sm">
            ...
            <p className="text-muted-foreground text-sm mb-6">{viewModel.zeroResultsSuggestionText}</p>
            <Link
              href="/"
              prefetch={true}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground text-xs font-semibold rounded-xl hover:bg-primary/90 transition shadow-md"
            >
